...
def KApersonal(request):
    # 数据最新日期
    latest_date = models.Account.get_latest_date()
    # 新开最新日期
    new_date = models.KAPersonal.get_latest_date()
    # 七天前的日期
    last_seven_day = latest_date + datetime.timedelta(days=-7)
    # 一天前的日期
    last_one_day = latest_date + datetime.timedelta(days=-1)
    ...
    if realname == '员工':
        ...
        userid = models.KAPersonal.objects.values('userid').filter(date=new_date,frame1=username)
        alluserid=[i['userid'] for i in userid]
        ...
        if alluserid:
            ...
            if len(alluserid) > 1:
                sql = "select p.id,t.username,p.company_name,p.depart,sum(t.allconsume) as consume from datacenter_kapersonal p left join datacenter_total t on p.userid=t.userid " \
                      "where t.date between '{}' and '{}' and t.userid in {} and p.date='{}' group by t.username".format(quarter_start_day,latest_date,tuple(alluserid),new_date)

                sql_more = "select p.id,t.username,t.date,sum(t.allconsume) as s from datacenter_kapersonal p left join datacenter_total t on p.userid=t.userid " \
                           "where t.date in {} and t.userid in {} and p.date='{}' group by t.username,t.date".format(date_filter, tuple(alluserid), new_date)
            ...
            alldata = models.KAPersonal.objects.raw(sql)
            more_data = models.KAPersonal.objects.raw(sql_more)
    ...
    if realname == '主管':
        userid = models.KAPersonal.objects.values('userid').filter(date=new_date, frame2=username)
        alluserid=[i['userid'] for i in userid]
        ...
        if alluserid:
            ...
            if len(alluserid) > 1:
                sql = "select p.id,p.frame1,sum(t.allconsume) as consume from datacenter_kapersonal p left join datacenter_total t on p.userid=t.userid " \
                      "where t.date between '{}' and '{}' and t.userid in {} and p.date='{}' group by p.frame1".format(quarter_start_day,latest_date,tuple(alluserid),new_date)

                sql_more = "select p.id,p.frame1,t.date,sum(t.allconsume) as s from datacenter_kapersonal p left join datacenter_total t on p.userid=t.userid " \
                           "where t.date in {} and t.userid in {} and p.date='{}' group by p.frame1,t.date".format(date_filter, tuple(alluserid), new_date)
    ...
    if realname == '经理':
        userid = models.KAPersonal.objects.values('userid').filter(date=new_date, frame3=username)
        alluserid=[i['userid'] for i in userid]
    ...
    if realname == 'all':
        ...
        userid = models.KAPersonal.objects.values('userid').filter(date=new_date)
        alluserid=[i['userid'] for i in userid]
        ...
        if alluserid:
            ...
            if len(alluserid) > 1:
                sql = "select p.id,p.frame1,p.depart,sum(t.allconsume) as consume from datacenter_kapersonal p left join datacenter_total t on p.userid=t.userid " \
                      "where t.date between '{}' and '{}' and t.userid in {} and p.date='{}' group by p.frame1".format(quarter_start_day,latest_date,tuple(alluserid),new_date)
    ...
    return render(request, 'datacenter/KApersonal.html',context)


def KApersonal_detail(request):
    # 数据最新日期
    latest_date = models.Account.get_latest_date()
    # 新开最新日期
    new_date = models.KAPersonal.get_latest_date()
    ...
    userid = models.KAPersonal.objects.values('userid').filter(date=new_date, frame1=name)
    alluserid = [i['userid'] for i in userid]
    sql = ''
    if len(alluserid) > 1:
        sql = "select p.id,p.userid,t.username,p.company_name,p.depart,sum(t.allconsume) as consume from datacenter_kapersonal p left join datacenter_total t on p.userid=t.userid " \
              "where t.date between '{}' and '{}' and t.userid in {} and p.date='{}' group by t.username order by consume desc".format(quarter_start_day,latest_date,tuple(alluserid),new_date)
